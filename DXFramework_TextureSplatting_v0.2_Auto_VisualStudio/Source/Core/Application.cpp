...
bool Application::Initialize(HINSTANCE hInstance)
{
    ...
    // ------------------------------------------------------------------------
    // 10. 선택 Feature: Texture Splatting Terrain 초기화
    //
    // 기존 HeightMap Geometry 생성 로직을 재사용하고,
    // Terrain Height / Normal을 이용해 Grass / Rock / Snow Weight를
    // Pixel Shader에서 자동 계산한다.
    // ------------------------------------------------------------------------
    ...
    // 경사도가 이 범위에 들어오면 Rock 비중이 증가한다.
    // Slope 0 = 평지, 1 = 수직에 가까운 면.
    ...
    if (!textureSplatTerrain_.Initialize(
            renderer_.GetDevice(),
    ...
    {
        return SetInitializationError(
            L"Texture Splatting Terrain 초기화",
            textureSplatTerrain_.GetLastErrorMessage());
    }

    // 각 테스트 대상을 좌우로 분리해 한 화면에서 비교하기 쉽게 한다.
    // 새 기능인 Texture Splat Terrain을 World 원점에 두어
    // 프로그램 실행 직후 바로 확인할 수 있게 한다.
    textureSplatTerrain_.GetTransform().Position =
    {
        0.0f,
        0.0f,
        0.0f
...
void Application::Render()
{
    ...
    // ------------------------------------------------------------------------
    // Texture Splatting Terrain
    //
    // Solid Rasterizer State와 Material Texture를 Feature 내부에서 관리한다.
    // ------------------------------------------------------------------------
    textureSplatTerrain_.Render(
        renderer_.GetContext(),
        view,
        projection);
