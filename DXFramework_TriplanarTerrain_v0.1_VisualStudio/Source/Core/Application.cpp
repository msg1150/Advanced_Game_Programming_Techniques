...
void Application::Update(float deltaTime)
{
    ...
    if (input_.IsKeyPressed(VK_F1))
    {
        const bool nextCullingState =
            !triplanarTerrain_.IsCullingEnabled();
    ...
    if (input_.IsKeyPressed(VK_F2))
    {
        const bool nextLODState =
            !triplanarTerrain_.IsLODEnabled();
...
void Application::Render()
{
    ...
    // Debug Overlay
    //
    // 숫자키 상태 + 공통 F1/F2 상태를 표시한다.
    // 통계는 최신 Triplanar Terrain을 우선 표시하고,
    // Triplanar가 꺼져 있으면 기존 QuadTree LOD Terrain 통계를 표시한다.
    // ------------------------------------------------------------------------
    ...
    const QuadTreeLODStats* stats =
        nullptr;

    const wchar_t* statsSource =
        nullptr;

    if (isTriplanarVisible)
    {
        stats =
            &triplanarTerrain_.GetStats();
    ...
    if (stats)
    {
        ...
        for (std::size_t index = 3u;
             index < stats->ActiveNodesPerLOD.size();
             ++index)
        {
            lod3PlusNodes +=
                stats->ActiveNodesPerLOD[index];
        }
        ...
        debugText
            ...
            << L" / "
            << stats->FullResolutionSurfaceTriangles
            << L"\n"
            << L"LOD Nodes 0 / 1 / 2 / 3+ : "
            << stats->ActiveNodesPerLOD[0]
            << L" / "
            << stats->ActiveNodesPerLOD[1]
            << L" / "
            << stats->ActiveNodesPerLOD[2]
            << L" / "
            << lod3PlusNodes;
    }
    else
