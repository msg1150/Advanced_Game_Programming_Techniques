    ...
    const DiskTerrainManager& terrain,float distance,float width,bool visible)const
{
    ...
    constexpr UIColor foreground=UITheme::StatsText;
    constexpr UIColor muted=UITheme::StatsMuted;
    ...
    constexpr float kStatsBackgroundOpacity=UITheme::StatsBackgroundOpacity;
    renderer.FillRoundRect({x,y,panelWidth,570.f},
                           {UITheme::PanelBackground.R,UITheme::PanelBackground.G,
                            UITheme::PanelBackground.B,kStatsBackgroundOpacity},8.f);
    renderer.StrokeRect({x,y,panelWidth,570.f},UITheme::PanelBorder);
    ...
    if(!terrain.GetError().empty())
        renderer.Text(L"타일 오류: VS 출력 창 확인",{x+14.f,y+532.f,342.f,24.f},
                      UITheme::StatsError,UIFont::Small);
}
