...
public:
    ...
    // Win32 Client Area 좌표 (UI HitTest는 DPI=96 Direct2D Pixel 좌표를 사용).
    int GetMouseX() const;
    int GetMouseY() const;

    // Windows TranslateMessage -> WM_CHAR: UI 숫자 입력용 프레임별 문자 목록.
    const std::wstring& GetTextCharacters() const;
    bool DidLoseFocus() const;
...
private:
    ...
    // 한 프레임 내 Down+Up이 모두 도착한 매우 짧은 클릭도 놓치지 않는다.
    bool leftPressedThisFrame_ = false;
    bool leftReleasedThisFrame_ = false;
