...
bool Application::Initialize(HINSTANCE hInstance)
{
    ...
    // 새 기능인 QuadTree LOD Terrain을 World 원점에 두어
...
void Application::Update(float deltaTime)
{
    ...
    if (input_.IsKeyPressed(VK_F1))
    {
        quadTreeLODTerrain_.SetCullingEnabled(
            !quadTreeLODTerrain_.IsCullingEnabled());
    }
    ...
    if (input_.IsKeyPressed(VK_F2))
    {
        quadTreeLODTerrain_.SetLODEnabled(
            !quadTreeLODTerrain_.IsLODEnabled());
    }
...
void Application::Render()
{
    ...
    //
    // LOD0 = 가장 세밀한 Leaf.
    // 숫자가 커질수록 더 성긴 Parent Node 표현이다.
    // ------------------------------------------------------------------------
    const QuadTreeLODStats& lodStats =
        quadTreeLODTerrain_.GetStats();

    std::uint32_t lod3PlusNodes = 0u;

    for (std::size_t index = 3u;
         index < lodStats.ActiveNodesPerLOD.size();
         ++index)
    {
        lod3PlusNodes +=
            lodStats.ActiveNodesPerLOD[index];
    }
    ...
    debugText
        ...
        << L"\n"
        << L"Active Nodes : "
        << lodStats.ActiveNodes
        << L"    Culled Nodes : "
        << lodStats.CulledNodes
        << L"    Draw Calls : "
        << lodStats.DrawCalls
        << L"\n"
        << L"Surface Triangles : "
        << lodStats.RenderedSurfaceTriangles
        << L" / "
        << lodStats.FullResolutionSurfaceTriangles
        << L"\n"
        << L"LOD Nodes 0 / 1 / 2 / 3+ : "
        << lodStats.ActiveNodesPerLOD[0]
        << L" / "
        << lodStats.ActiveNodesPerLOD[1]
        << L" / "
        << lodStats.ActiveNodesPerLOD[2]
        << L" / "
        << lod3PlusNodes;
