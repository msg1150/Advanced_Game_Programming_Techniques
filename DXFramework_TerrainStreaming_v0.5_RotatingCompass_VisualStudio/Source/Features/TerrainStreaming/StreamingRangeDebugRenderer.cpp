    ...
    const XMMATRIX& terrainWorld)
{
    ...
    // ---------------------------------------------------------------------
    // 회전된 Chunk의 네 모서리를 모두 계산해 화면에 맞게 배치한다.
    // 축 정렬 Bounds의 min/max 두 점만 돌리면 회전 후 잘리므로 금지한다.
    // 원은 회전해도 원이므로 Focus 기준 ±UnloadRadius까지 함께 포함한다.
    // ---------------------------------------------------------------------
    float minU = -unloadRadius_ - 0.2f;
    float maxU = +unloadRadius_ + 0.2f;
    float minV = -unloadRadius_ - 0.2f;
    float maxV = +unloadRadius_ + 0.2f;
    for (const auto& chunk : chunks_)
    {
        ...
        const ScreenPoint corners[4] =
        {
            toMap(x0,z0), toMap(x1,z0), toMap(x1,z1), toMap(x0,z1)
        };
        for (const ScreenPoint& p : corners)
        {
    ...
    const float worldSide = std::max(maxU-minU, maxV-minV);
    if (!(worldSide > 0.f)) return;
    const float centerU = 0.5f * (minU + maxU);
    const float centerV = 0.5f * (minV + maxV);
    const float scale = mapSide / worldSide;

    const auto toScreen = [&](float x, float z) -> ScreenPoint
    {
        const ScreenPoint p = toMap(x,z);
        return {mapLeft + 0.5f*mapSide + (p.X-centerU)*scale,
                mapTop  + 0.5f*mapSide + (p.Y-centerV)*scale};
    };
    ...
    context->DrawIndexed(triangleCount,0u,0);
    if(indices.size()>triangleCount)
    {
        context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_LINELIST);
        context->DrawIndexed(static_cast<UINT>(indices.size()-triangleCount),triangleCount,0);
    }
