...
private:
    ...
    // F1/F2 Terrain 상태와 숫자키 토글 상태를 화면에 표시하는 공용 Debug Overlay.
...
// --------------------------------------------------------------------
// 공용 Utility: 화면에 배치된 Showcase 항목 표시 상태 관리
//
// 숫자키 1~0을 고정 슬롯으로 사용한다.
// 현재 단계에서는 Cube / Perlin / HeightMap / QuadTree LOD를 배정하고,
// 나머지는 이후 단계(예: Triplanar Terrain)용 예약 슬롯으로 남겨둔다.
// --------------------------------------------------------------------
ShowcaseVisibilityController showcaseVisibilityController_;
    ...
    // --------------------------------------------------------------------
    // 선택 기능: HeightMap Terrain
    //
    // HeightMap 이미지 기반 Terrain이다. Perlin Terrain과 동일하게
    // 기존 Framework 구현을 직접 수정하지 않고 독립 Feature 객체로 연결한다.
    // --------------------------------------------------------------------
    HeightMapTerrain heightMapTerrain_;

    // --------------------------------------------------------------------
    // 선택 기능: QuadTree LOD Terrain
    //
    // 이전 HeightMap / Texture Splatting 기능을 그대로 재사용하고,
    // 새 Features/QuadTreeLOD 폴더에서 Frustum Culling + Distance LOD를
    // 독립적으로 적용한다.
    //
    // 이 기능을 제거할 때는:
    // - 이 멤버
    // - Application.cpp의 Initialize / F2 / Render 연결부
    // - Source/Features/QuadTreeLOD
    // 만 제거하면 이전 기능들은 그대로 유지된다.
    // --------------------------------------------------------------------
    QuadTreeLODTerrain quadTreeLODTerrain_;

// --------------------------------------------------------------------
// 선택 기능: Triplanar Terrain
//
// 기존 HeightMap / QuadTree LOD 구현은 그대로 재사용하고,
// Material / Shader만 독립 Triplanar Feature로 교체한다.
//
// 제거하려면 이 멤버와 Application 연결부,
// Source/Features/TriplanarTerrain 및 Shaders/TriplanarTerrain만
// 제거하면 된다.
// --------------------------------------------------------------------
TriplanarTerrain triplanarTerrain_;

    // 신규 Chunk Feature: 기존 Terrain 구현과 독립된 렌더링/LOD 관리 객체.
    TerrainChunkManager terrainChunkManager_;

    // 선택 기능: HeightMap 기반 GPU Chunk Streaming (기존 Chunk Feature 불변).
    TerrainStreamingManager terrainStreamingManager_;

    // 큰 Debug Overlay는 F3 입력으로만 켜고 끈다. 기본값은 OFF.
