    ...
    bool compact)
{
    ...
    const D2D1_RECT_F backgroundRect =
        D2D1::RectF(
            12.0f,
            12.0f,
            compact ? 207.0f : 760.0f,
            compact ? 53.0f : 329.0f);
    ...
    const D2D1_RECT_F textRect =
        D2D1::RectF(
            24.0f,
            20.0f,
            compact ? 202.0f : targetSize.width - 24.0f,
            compact ? 50.0f : 324.0f);
...
bool DebugTextRenderer::CreateTargetResources()
{
    ...
    // ------------------------------------------------------------------------
    // 최신 Windows SDK 권장 방식에 맞춰
    // SwapChain이 연결된 실제 Window의 DPI를 GetDpiForWindow()로 얻는다.
    // ------------------------------------------------------------------------
