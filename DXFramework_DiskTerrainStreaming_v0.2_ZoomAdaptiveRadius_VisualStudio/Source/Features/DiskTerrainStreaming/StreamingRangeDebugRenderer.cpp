    ...
    const XMMATRIX& terrainWorld)
{
    ...
    // ---------------------------------------------------------------------
    // 대형 지형에서는 전체 월드를 미니맵 하나에 넣으면 로딩 반경이
    // 몇 픽셀로 축소된다. 따라서 Focus 주변만 고정 배율로 표시한다.
    // 지형이 카메라와 함께 회전해도 녹색/주황 반경 크기는 일정하다.
    // ---------------------------------------------------------------------
    // 화면 확대 때 Mini Map까지 과하게 Zoom Out하지 않도록 하한을 둔다.
    // Zoom 거리 증가에 따른 두 원의 실제 크기 변화가 눈에 보인다.
    const float viewRadius = std::max(85.0f,unloadRadius_ * 1.12f);
    const float scale = mapSide / (2.0f * viewRadius);
