    ...
    const TriplanarMaterialDesc& material,const DiskTerrainSettings& settings)
{
    ...
    if(!device || !context || !(settings.LoadRadius>0.f) ||
       ...
       (settings.ZoomRadius.Enabled &&
        !ZoomStreamingRadiusPolicy::IsValid(settings.ZoomRadius)))
    {error_=L"Disk Terrain 설정 오류";return false;}
    settings_=settings;device_=device;context_=context;
    activeRadii_={settings.LoadRadius,settings.UnloadRadius};
...
void DiskTerrainManager::Update(const XMFLOAT3& target,float cameraDistance)
{
    if(!initialized_)return;
    focus_=target;

    // Zoom 변화에 따라 이번 프레임의 실제 Load/Unload 반경을 계산한다.
    // 반경 정책은 별도 Header의 순수 함수로 격리되어 있다.
    activeRadii_=ZoomStreamingRadiusPolicy::Calculate(
        cameraDistance,settings_.LoadRadius,settings_.UnloadRadius,
        settings_.ZoomRadius);
...
float DiskTerrainManager::GetMaximumLoadRadius()const{
    return settings_.ZoomRadius.Enabled
        ? settings_.ZoomRadius.MaximumLoadRadius:settings_.LoadRadius;
}
